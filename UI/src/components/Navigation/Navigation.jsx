...
const Navigation = ({ variant = "default" }) => {
  // TODO working of useSelector here
  const cartLength = useSelector(countCartItems);
  // navigate based on some logic (e.g., after a form submission or API call).
    ...
    <nav className='flex items-center py-6 px-16 justify-between gap-20 custom-nav'>
      ...
      {variant === "default" &&
        <div className='flex flex-wrap items-center gap-10'>
          {/* Nav items */} 
          {/* isActive: A boolean provided by react-router-dom indicating if the current route matches the to path in. */}
          <ul className='flex gap-14 text-gray-600 hover:text-black'>
            {/* NavLink: navigation menus where you want to indicate the current page. */}
            {/* isActive: boolean indicating whether the to path matches the current route. if true active-link class in css file we be applied else nothing will be applied  */}
            {/* Link: Does not include active styling capabilities. */}
