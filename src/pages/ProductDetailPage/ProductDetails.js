...
const ProductDetails = () => {
  // to get id of product since useLoaderData() is not working
  const { id } = useParams();
  const products = content?.products;
  const product = products.filter((p) => p?.id === parseInt(id)).at(0);
  // const { product } = useLoaderData();
  const [image, setImage] = useState();
  const [breadCrumbLinks, setBreadCrumbLink] = useState([]);
  // TODO what does dispatch do
  const dispatch = useDispatch();
  // TODO what does useSelector do, it not working currently i guess
  ...
  useEffect(() => {
    ...
    getAllProducts(product?.category_id, product?.type_id).then(res => {
      if (res) {
        const excludedProduct = res?.filter((item) => item?.id !== product?.id);
        setSimilarProducts(excludedProduct);
      } else {
        // If we get no similar product from DB
        const exclude = products?.filter((item) => item?.id !== product?.id);
        setSimilarProducts(exclude);
  ...
  // TODO use call back understanding
  const addItemToCart = useCallback(() => {
    ...
    if (!selecteSize) {
      ...
      // here .[0] is use since size is an array/list and only one element is left after
      const selectedVariant = product?.size?.filter((s) => s === selecteSize)?.[0];
      console.log("selected ", selectedVariant);
      // if (selectedVariant?.stockQuantity > 0) {
      // TODO dispatch use ? and addItemToCartAction function working
      // dispatch: send actions/functions to the Redux store which allows you to trigger state changes in Redux store
  ...
  const colors = useMemo(() => {
    ...
    // return list on unique colors
    const colorSet = _.uniq(_.map(product?.color));
    return colorSet
  ...
  const sizes = useMemo(() => {
    ...
    // return list on unique sizes
    const sizeSet = _.uniq(_.map(product?.size));
    return sizeSet
          ...
          <div className='flex flex-col md:flex-row'>
            <div className='w-[100%] md:w-[20%] justify-center h-[40px] md:h-[420px]'>
              ...
              <div className='flex flex-row md:flex-col justify-center h-full'>
                {
                  // TODO can make this a horizental scroller
        ...
        <div className='w-[60%] px-10'>
          ...
          <div className='flex flex-col py-2'>
            <div className='flex gap-2'>
              ...
              {/* {'Size Guide ->'} has curly braces since > is not allowed without curly braces  */}
          ...
          <div className='mt-2'><SizeFilter onChange={(values) => {
            // if values?.[0] is null or undefined set ''
